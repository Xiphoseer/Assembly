                              ...
                              const std::string& filter )
{
    ...
    if (dpdf != NULL)
    {
        ...
        {
            if (epdf->d_type == DT_REG && fnmatch(filter.c_str(), epdf->d_name, 0) == 0)
            {
...
std::vector<std::string>
fs::dirs_in_dir            ( const std::string& path,
                             const std::string& filter )
{
    DIR *dpdf;
    struct dirent *epdf;
    ...
    if (dpdf != NULL)
    {
        while (epdf = readdir(dpdf))
        {
            if (epdf->d_type == DT_DIR && fnmatch(filter.c_str(), epdf->d_name, 0) == 0)
            {
                files.push_back(std::string(epdf->d_name));
            }
        }
